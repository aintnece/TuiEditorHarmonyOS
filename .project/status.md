...
**Last updated**: 2026-06-23 (Phase 7 Batch A(MdPreview 代码高亮) + 主题修复 代码完成 ⏳ 待真机复验。Phase 6 全部验证 ✓)
...
**Phase 7 代码语法高亮 — Batch A (MdPreview)** 代码完成 ⏳ 待真机验证。
...
- **Batch B (WYSIWYG) 待做**：editor.html 注册 tui 插件 `toastui.Editor.plugin.codeSyntaxHighlight`（资产已在 `rawfile/tui-editor/` plugin-all.js + css）。
- 真机验证点：Markdown 预览写带语言代码块→关键字着色；切暗/亮主题对应；改内容仍高亮。
- **首轮真机暴露 2 问题，已修（待复验）**：① 代码无色=vendoring 的 prism.js minified 直接 cat 粘连（`}()/**` → `/` 除号 + `**` 幂运算）语法坏，Hermes 加 `\n;\n` 分隔重拼（node 验证 OK）；② 主题按钮无反应=EditorPage 双重 toggle 抵消（已去重）+ MdPreview 不听 stateChange（已加监听整页重载，injectPrism 改读 core 权威主题解时序）。spec: `.project/7a-fix-theme-spec.md`。
...
候选方向（按价值/独立性排，待用户选）：
- **代码语法高亮**（tui code-syntax-highlight 插件）— 代码块着色，可见度最高
- **查找替换** — 搜索栏 + 高亮 + 上/下一处 + 替换（独立大批次，两模式各自实现）
- **Phase 8：解析器跑 CommonMark spec 测试套件** — 解析器正确性保障
