...
**Phase 6.2 缩进 / 减少缩进（indent / outdent）** — 设计中，准备交 CC。
- 三处改动 + 2 新图标：Toolbar.ets 加「缩进/减少缩进」两按钮 → exec('Indent')/exec('Outdent')；Editor.ts mapToTui 加 Indent→indent / Outdent→outdent（tui 引擎已确认有这两命令）；MarkdownCommands.ts 新增 IndentCommand（行首加 2 空格）/ OutdentCommand（删行首至多 2 空格或 1 个 tab）并注册；新建 `tui_indent.svg`/`tui_outdent.svg`（Feather 风格）。
