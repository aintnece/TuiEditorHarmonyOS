...
**Last updated**: 2026-06-23 (Phase 6.1 标题级别 H1-H6 代码完成 ⏳ 待真机验证。Phase 5 + 图库插图已真机验证 ✓)
...
**Phase 6.1 标题级别 H1-H6** — 代码完成 ⏳ 待真机验证。
- 改动仅 `components/Toolbar.ets`：新增 `@Builder headingBtn()`（视觉同 iconBtn + 保留 bindPopup tooltip，用 `.bindMenu(this.headingMenu)` 替代 onClick）+ `@Builder headingMenu()`（6×MenuItem，content「标题 N」+ labelInfo「HN」，各 exec('Heading','1'..'6')）。
- 后端无需改动：Editor.ts mapToTui + MarkdownCommands.HeadingCommand 早已读 args[0] 支持任意级别。
- spec: `.project/6.1-heading-levels-spec.md`。
- 真机验证点：两模式各测（光标停某行→点标题→选 H3→变 ### / H3 样式）；确认 bindPopup(hover)+bindMenu(click) 共存无冲突、菜单不被裁切。
