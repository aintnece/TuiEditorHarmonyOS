...
**Last updated**: 2026-06-23 (Phase 5 全部完成并真机验证 ✓ — WYSIWYG 引擎/容器/输入/双向桥/模式·文件同步/工具栏全套命令/tooltip 全通过。下一步：图片插入支持系统图库/文件选择 + Phase 6)
...
**图片插入接系统图库（方案 A）— 已实现，待真机验证。** PhotoViewPicker 选图 → FileService.copyImageToSandbox 复制进 `filesDir/userimg/` → imageUrl 填相对路径 `userimg/<name>` → WwEditor + MdPreview 两个 onInterceptRequest 加 `userimg/` 分支从 filesDir 读出。规格 `.project/image-picker-spec.md`。**真机风险点**：picker import(`@kit.CoreFileKit`)是否编译、`fs.copyFileSync(srcUri,dest)` 是否接受选择器 URI。

## Next

- 图片插入：系统图库（方案 A）已实现待验；后续可加 DocumentViewPicker（任意文件）+ 图片大小校验/压缩
