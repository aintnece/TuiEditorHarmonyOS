...
**Last updated**: 2026-06-23 (Phase 7 Batch B(WYSIWYG 代码高亮) 代码完成 ⏳ 待真机验证。Batch A 已验证 ✓。Phase 6 全部验证 ✓)
...
**Phase 7 代码语法高亮 — Batch B (WYSIWYG) 进行中**。Batch A (MdPreview) ✅ 真机验证通过（高亮/主题切换/配色跟随/实时更新均确认；高亮需在 ``` 后指定语言，属正常 Prism 行为）。
...
- **Batch B (WYSIWYG) 代码完成 ⏳ 待真机验证**：editor.html 加插件 CSS link + 插件 JS（editor-all 之后）+ 守卫式 plugins 数组（`toastui.Editor.plugin.codeSyntaxHighlight`，取不到降级防白屏）+ `plugins:plugins`。WwEditor 拦截器自动服务资产（未改）；桥/change 抑制零改动。spec: `.project/7b-codehl-wysiwyg-spec.md`。
