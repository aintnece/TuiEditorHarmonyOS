...
**Last updated**: 2026-06-24 (Phase 8.2e-2 Entity 数字字符引用解码完成。基线 exact 67.64% (441/652)，Entity 5→12，全 26 节 0 回归)
...
**（无活动批次）** — Phase 8.2e-2 Entity 数字引用完成，8.2e 其余子簇（命名实体大表 / Autolinks / Hard breaks 反斜杠换行 / Raw HTML / HTML blocks）待选。
...
- **8.2e 零碎**（剩余最大杠杆，逐子簇累加）：✅ **8.2e-1 Code spans**（9→18）、✅ **8.2e-2 Entity 数字引用**（Entity 5→12）已完成（commit 见下）。待选子簇：命名实体大表(Ex25/32/33/34，需 vendoring ~2125 项表 + 链接 dest/title/info-string 上下文解码) / Autolinks(9) / Hard breaks 反斜杠换行+行尾空格(Ex634/637/639/645) / inline Raw HTML(14，含 Emphasis 剩的 Ex475-477，需 HTML 标签文法) / HTML blocks(24，块级大功能) / Setext(7)。
