...
**Last updated**: 2026-06-23 (Phase 6.1 标题级别 H1-H6 ✅ 真机验证通过。Phase 5 + 图库插图已验证 ✓)
...
**Phase 6.2 缩进 / 减少缩进（indent / outdent）** — 代码完成 ⏳ 待真机验证。
- 三处代码 + 2 新图标（5 文件）：Toolbar.ets 加「缩进/减少缩进」两按钮（代码块后）；Editor.ts mapToTui 加 Indent→indent / Outdent→outdent；MarkdownCommands.ts 新增 IndentCommand（行首+2空格）/ OutdentCommand（删行首 tab×1 或空格≤2，选区夹紧防越界）并注册；新建 `tui_indent.svg`（右向箭头）/ `tui_outdent.svg`（左向箭头）。
- ⚠️ Hermes 修正两处资产问题：(1) CC 误把 SVG 建到仓库根 `resources/`，已移到正确的 `entry/src/main/resources/base/media/` 并删除多余目录；(2) CC 的 indent 图标方向画反（左向），已改 polyline 为右向，与 outdent 成镜像对。
- spec: `.project/6.2-indent-outdent-spec.md`。
- 真机验证点：Markdown 模式行首增减空格；WYSIWYG 模式在列表项内升降级；两新按钮图标方向/ tooltip 正常。
