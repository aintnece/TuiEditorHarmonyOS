...
**Last updated**: 2026-06-23 (Phase 7 代码语法高亮 双模式 ✅ 真机验证通过。遗留: WYSIWYG 暗色用 tui 调色板, 与应用 ThemeService 调色板不完全一致, 待议)
...
**Phase 7 代码语法高亮 ✅ 双模式真机验证通过**（Batch A MdPreview + Batch B WYSIWYG）。
- 遗留设计议题（待用户定方向）：WYSIWYG 暗色用 tui 自带暗色主题（toastui-editor-dark + prism-tomorrow），与应用 ThemeService 28 token 调色板不完全一致；MdPreview 暗色也是自有调色板。三处暗色（ArkUI 外壳 / MdPreview / WYSIWYG）各自为政。候选：①注入 ThemeService 调色板覆盖 WebView CSS 变量统一外观（一致性最佳）②改用 HarmonyOS 系统深色(Web darkMode=Auto)——跟随 OS 非应用调色板，架构变动 ③轻量覆盖关键几色。
