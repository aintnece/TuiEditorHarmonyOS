...
**Last updated**: 2026-06-25 (Phase 9.1+9.2 ✅ 真机已过(统一/白线4d8851c/三选一跟随系统/编译修colorMode 2b8c04d)；Phase 9.3 首帧主题修复【已实现·待真机】commit d06dc48。Phase 8 解析器暂停于 exact 92.33%)
...
**（无活动批次）** — Phase 8.2j 多行链接引用定义 完成（Link ref defs +6）。剩余多为 Links bracket 栈（13，最难）/ ref-def 上下文（212/213/218，8.2k）/ tight-loose（4）/ Tab（5）/ Images（6）。

### ✅ Phase 8.2j — 多行链接引用定义（multi-line link reference definitions，CommonMark 4.7）
- 改 3 文件：(A) `Inlines.ts` 新增 `parseLinkRefDefBlock(lines,start)` + `parseSingleRefDef(block,pos)`（拼连续非空行成 block 字符串 → 基于位置反复解析 ref-def，复用现有 `parseAngleDest`/`parseBareDest`/`parseTitle`/`resolveBackslashEscapes`/`normalizeUri`；label 用 `block.indexOf(']:')` 可跨行、dest/title 各允许至多一个前导 `\n`、title 后非空白则丢弃 title 回退 posAfterDest、支持一 block 多定义；具名 class LinkRefDefBlockResult/SingleRefDefResult）；`parseTitle` 删 2 处 `if(ch==='\n')return null`（title 跨行）；**保留旧单行 `parseLinkRefDefLine`**。(B) `ToastMark.ts` 预扫 `for 逐行` → `while + parseLinkRefDefBlock`，按 linesConsumed 推进。(C) `Blocks.ts` tryParseBlock 单行消费 → 多行：`save()` 字符级 peek 连续非空行 → parseLinkRefDefBlock → `nextLine()×linesConsumed`，不匹配 `restore()`。无正则。
- **基线 91.41% → 92.33%（exact 596→602，+6）**；**Link ref defs 16→22**；**全 26 节 0 回归**（逐节净增量 +6=+6 自洽）；error 0；**hang 0**（linesConsumed 换算正确，harness 正常跑完）；cosmetic 0。6 条 oracle（193/195/196/198/208/217）全 PASS。
- 🎯 **关键经验**：① **多行块构造可复用「text+pos」接口的现有解析器**——把连续行拼成 block 字符串，dest/title 解析器（接受 text+pos）天然跨 `\n`，不必重构、不碰 parseParagraph；② `parseTitle` 删 `\n` 限制血缘到**所有 link/title**，但全节回归验证 **Links 69/Code spans 21/Images 16 全不变**（inline link 的 `findClosingBracket` 在 `\n` 处先拒绝，不会走到 title 跨行路径——CC 论证经 Hermes 回归证实）；③ `ParseState.save()` 返回**字符偏移**（pos）、`input`/`len` 是 public 字段，可字符级 peek 多行。
- 三方核对一致（CC / Hermes 亲跑 harness=602 / 全节回归脚本 REGRESSIONS NONE + 净增量自洽 6=6 + 血缘节 Links/Code spans/Images/Paragraphs 全不变）+ ArkTS grep 0 命中（具名 class、字符串/数组下标合法、无正则/匿名字面量/Record/spread/any/对象属性下标）。spec: `.project/8.2j-multiline-linkref-spec.md`。**DevEco 真机编译通过 ✅（含 parseTitle 广血缘改动终验）。**

### ✅ Phase 8.2i — 带前导空格的列表 marker + 内容列 W（CommonMark 5.2 簇 B）
- `Blocks.ts` 只改 3 处（**parseList 主体 0 改动**）：(A) `parseBulletMarker` 开头数前导空格 p（≤3，≥4→null=indented code），marker 在 `line[p]`，`contentCol` 改**绝对列**=p+markerWidth+后空白W；(B) `parseOrderedMarker` 同理加前导空格 p，数字从 `line[p]` 起、`num=parseInt(substring(p,i))`、markerWidth=p+位数+1、contentCol 含 p；(C) `isNonEmptyListMarker` 加「ordered marker 仅 `num===1` 才可打断段落」（防 Ex304 `14.` 误打断）。无正则。
- **基线 88.80% → 91.41%（exact 579→596，+17，本会话最大单批，突破 90%）**；**List items 38→48 满分**、**Lists 17→22**、连带 **Indented code 9→10**、**Tabs 5→6**；**全 26 节 0 回归**（逐节净增量 +10/+5/+1/+1=17 与总增量完全一致）；error 0；**hang 0**；cosmetic 0。15 条 oracle（257/258/271/286/287/288/290/291/295/297/304/310/311/312/313）全 PASS。
- 🎯 **关键设计经验（低风险高 ROI 的根本）**：最高风险批次（marker 识别 = 最广血缘，影响入口/parseList/parseParagraph/isLazyContinuable 全调用点），但**parseList 主体一行没碰**——因 `contentCol` 改为**绝对列** + 现有 `substringByColumn`/`countIndentColumns`/`stripIndentCols` 本就是绝对列语义，归属（`indentCols >= contentCol`）/ 嵌套 / 同级判定**自动正确**。**把"前导缩进"统一进 contentCol 绝对列，是不碰 parseList 主体就修一大片的关键。**
- ⚠️ **第四次「marker 识别变化 → 段落边界检测必须同步」联动**：marker 认前导空格后更多行成 marker，`isNonEmptyListMarker` 必须加 ordered start=1 限制（CommonMark：ordered list 仅 start=1 才能打断段落），否则 Ex304 类回归。Hermes 全节回归验证 Paragraphs 7/Block quotes 24/Setext 26/Thematic breaks 18 全不变。入口顺序确认：line53 indented code(≥4空格) 在 marker 检查前，前导空格 marker 只认 ≤3。
- 三方核对一致（CC / Hermes 亲跑 harness=596 / 全节回归脚本 REGRESSIONS NONE + 逐节净增量自洽 17=17）+ ArkTS grep 0 命中（具名 class、字符串下标合法、无正则/匿名字面量/Record/spread/any/对象下标）。spec: `.project/8.2i-leading-space-marker-spec.md`。**DevEco 真机编译通过 ✅。**
...
- 三方核对一致（CC / Hermes 亲跑 harness=579 / 全节回归脚本 REGRESSIONS NONE + 净增量自洽）+ ArkTS grep 0 命中（isNonEmptyListMarker 复用具名 class/helper、字符串下标合法、无正则/匿名字面量/Record/spread/any/对象下标）。spec: `.project/8.2h-empty-list-items-spec.md`。**DevEco 真机编译通过 ✅。**
...
### ✅ Phase 9.3 — 启动期/首帧主题同步修复【已实现·待真机验证】
- 真机发现 2 个首帧 bug（9.2 真机 5 点已过、切换后正常 → 订阅链路对，缺初始值同步）。commit `d06dc48` push main。spec `.project/9.3-init-theme-fix-spec.md`。
- 问题1 启动页 `Index.ets` 全硬编码浅色、没接 ThemeService → 接入(@State theme + onThemeChange 订阅) + 8 处颜色 token 化(toolbarFg/toolbarBg/borderColor/toolbarActive/editorFg/editorPlaceholder/editorBg)。
- 问题2 首次进文档预览/编辑浅色而外壳深 → `EditorPage` L385 `opts.useDarkTheme` 硬编码 false 改取 `getTheme().isDark`(修预览 renderer 首帧) + `Editor.factory` 后加 `this.theme = getTheme()`(强制 @State 首帧同步，MdEditor/MdPreview/外壳 themeColors 首帧即正确)。
- 审 diff 已过：ArkTS(具名回调) + token 映射 + 仅改颜色取值与首帧同步、不动布局/逻辑/调色板。

### ✅ Phase 9.2 — 跟随系统（深浅色三选一）【✅ 真机已过（5 点全验）+ 编译补修 colorMode 类型 2b8c04d】
- 加「跟随系统」第三态 + 主题切换 UI 改 bindMenu 三选一（浅色/深色/跟随系统，✓ 标当前项），默认跟随系统。commit `6b6a848` push main。spec `.project/9.2-follow-system-spec.md`。
- 改 5 文件：① `ThemeService.ts` ThemeMode+System、userMode+systemIsDark 双态、setMode/setSystemColorMode(system-only 守卫)/getUserMode/applyEffective、默认 system、savePreference 存 userMode；② `EditorCore.ts` 构造订阅 onThemeChange 统一重建(renderer + emit stateChange)，toggleTheme/setMode 不再重复重建；③ `Editor.ts` setMode/getUserMode 透传；④ `app.ets` onConfigurationUpdate 监听系统亮暗 + onCreate 喂初始 `this.context.config.colorMode`；⑤ `EditorPage.ets` 标题栏主题按钮 bindMenu 三选一 + themeModeLabel/menuLabel(✓)/pickMode。
- 架构点：把「重建预览 renderer + emit stateChange」从命令式(原 toggleTheme 内)改订阅式(EditorCore 订阅 ThemeService)，手动切换与系统自动切换走同一链路 → 三处同步。
- 审 diff 已过：ArkTS(if 链/无 Record/具名类型/显式返回，bindMenu 对象字面量同 6.1) + 订阅无重复重建/无 echo-loop + 系统 colorMode API 用法正确。
- ⚠️ 待编译确认：`Configuration` 从 @kit.AbilityKit 导入，若 DevEco 报错改 @ohos.app.ability.Configuration。⏳ 真机清单见 spec §9。

### ✅ Phase 9.1 — 主题统一（深浅色单一数据源）【✅ 真机已过 + 白线修复 commit 4d8851c】
- 痛点（用户原话）：WYSIWYG 单独调的深色与界面深色灰度不一致，像半成品。根因 = WYSIWYG 用第三方 `toastui-editor-dark.css`（473 行硬编码蓝灰阶 #232428/#121212/#67ccff），与外壳 VSCode 调色板（#1e1e1e/#d4d4d4/#569cd6）两套灰度；预览侧本就≈外壳。
- 方案 **B 全量变量化**：三处暗色统一为 **12 个 `--ed-*` CSS 变量**，单一数据源 = ThemeService。改 5 文件：① `ThemeService.ts` +3 token(editorCodeBg #2d2d2d / editorBorderSubtle #333333 / editorAccentHover #4b8bc4) + `toCssVars()` 序列化(纯字符串拼接,ArkTS 安全)；② `toastui-editor-dark.css` 全量变量化(134 处 `var(--ed-*,#fallback)`，零硬编码残留，base64 SVG/prism/保留 rgba 未动)；③ `editor.html` `setTheme(dark,varsCss)` 注入 `:root` 到 `#ed-theme-vars`；④ `WwEditor.ets` themeHandler/applyInitial 传 `toCssVars()`；⑤ `Renderer.ts` renderFullPage 内联 `:root` + body 读 var。行内代码中性化、prism 语法色保留、base64 SVG 图标本批未动、跟随系统未做。
- 审 diff 已过：ArkTS(纯字符串拼接/具名 class/无 Record/无匿名对象字面量) + dark.css 映射零误配 + base64 完好 + 一致性(Renderer 内联值 == toCssVars 产出)。**commit 215d976 已 push main**。spec: `.project/9.1-theme-unify-spec.md`。
- ⏳ **待用户真机验证**（验证清单见 spec §10）：深色下 WYSIWYG/预览/外壳三处灰度一致、行内码中性、prism 彩色可读、深↔浅实时切换。真机偏差只需调 DarkTheme token 值或某处 var 映射（变量已贯通，无需重构）。
- 下一步：真机过后做 **Phase 9② 跟随系统**（监听 ConfigurationConstant.ColorMode → ThemeService.isDark + 浅/深/跟随系统三选一设置 UI）。

## Next

**🎯 当前焦点 = Phase 9 主题统一**：9.1 统一 + 9.2 跟随系统 ✅ 真机已过；9.3 首帧主题修复已实现待真机(见上)。真机过后 Phase 9 全收官。Phase 8 解析器暂停于 exact 92.33%（下方存档）。

**Phase 8.2 — 解析器修复（已暂停于 92.33%，存档）**（按 ROI 排，每批 CC 修 → 重跑 harness → 用 baseline.json 量增量）：
