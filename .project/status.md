...
**Last updated**: 2026-06-24 (Phase 8.2e-5 normalizeURI 百分号编码完成。基线 exact 71.01% (463/652)，Links/Link-ref/Autolinks/Code spans/Backslash 各节增，全 26 节 0 回归)
...
**（无活动批次）** — Phase 8.2e-5 normalizeURI 完成，8.2e 其余子簇（命名实体大表 / inline Raw HTML / HTML blocks / Setext / Ex645）待选。
...
- **8.2e 零碎**（剩余最大杠杆，逐子簇累加）：✅ **8.2e-1 Code spans**（9→18）、✅ **8.2e-2 Entity 数字引用**（5→12）、✅ **8.2e-3 Autolinks**（10→18）、✅ **8.2e-4 Hard breaks**（9→12）、✅ **8.2e-5 normalizeURI**（Links/Link-ref/Autolinks 等 +8）已完成（commit 见下）。待选子簇：命名实体大表(Ex25/32/33/34 + dest 双因，Hermes vendoring ~2125 项表 + CC 写解码器 + dest/title/info-string 上下文) / inline Raw HTML(14，含 Emphasis 剩的 Ex475-477、Hard breaks 剩的 Ex642/643) / HTML blocks(24，块级大功能) / Setext(7) / Ex645(块末尾 trim，Blocks 层)。
