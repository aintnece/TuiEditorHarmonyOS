...
**Phase 5 批次 3: ArkTS↔JS 桥** — ✅ 已实现（onChange 回写 core + exec 原语 + registerJavaScriptProxy），真机已验证「打字→切 Markdown 看到改动」。**含真机修复**：①加载文件误标脏 ②WYSIWYG 切文件不刷新（loadToken）③**异步 change 根因修**：tui.editor 的 change 异步触发，同步 suppressChange 拦不住 → 导致误标脏 + 延迟 onChange 写回旧内容串台（幽灵文字）；改用 `lastSetMd` 内容比对（cur===lastSetMd 即程序化设值，忽略）。④**Markdown 模式同因**：MdEditor 的 `TextArea.onChange` 在 editorText 被程序化改变时也触发 → 误标脏；同样用内容比对守卫（`value !== core.getMarkdown()` 才回写）。**待真机复验**。详见 `.project/batch3-fix-spec.md`。
