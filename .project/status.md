...
**Phase 5 批次 3: ArkTS↔JS 桥** — ✅ 已实现（onChange 回写 core + exec 原语 + registerJavaScriptProxy），真机已验证「打字→切 Markdown 看到改动」。**含 2 个真机修复**：①加载文件误标脏（setMarkdown isDirty=false + EditorPage 脏标志读 core.state）②WYSIWYG 下切文件内容不更新（WwEditor @Prop@Watch loadToken 触发重灌）。**待真机复验这 2 修复**。详见 `.project/active-task.md` / `batch3-fix-spec.md`。
