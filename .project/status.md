...
- **8.2a 快赢**（高置信，单/少 bug 翻一片）：① 图片 alt 吞首字符 off-by-one（Images 21/22 全灭根因，预期大涨）；② 反斜杠转义死循环（修 Inlines，干掉 HANG #12 crash，修好后从 skip.json 移除该条）；③ Tabs `2空格+tab` 缩进代码块识别。
- **8.2b 强调/加粗**（Emphasis 86/132，最大簇）：补 CommonMark delimiter-run 算法。
- **8.2c 链接**（Links 69/90 + Link ref def 22/27）：URL 归一化 + 引用式链接解析。
- **8.2d 列表**（List items 42/48 + Lists 22/26）：tight/loose、`<ol start>`、嵌套。
