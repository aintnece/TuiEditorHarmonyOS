...
**Last updated**: 2026-06-24 (Phase 8.2e-1 Code spans 反引号 run 等长匹配完成。基线 exact 66.56% (434/652)，Code spans 9→18，全 26 节 0 回归)
...
**（无活动批次）** — Phase 8.2e-1 Code spans 完成，8.2e 其余子簇（Entity refs / Autolinks / Hard breaks 反斜杠换行 / Raw HTML / HTML blocks）待选。
...
- **8.2e 零碎**（剩余最大杠杆，逐子簇累加）：✅ **8.2e-1 Code spans 完成**（9→18，commit 见下）。待选子簇：Entity/numeric refs(12，数字引用快赢 + 命名实体需表) / Autolinks(9) / Hard breaks 反斜杠换行+行尾空格(Ex634/637/639/645) / inline Raw HTML(14，含 Emphasis 剩的 Ex475-477，需 HTML 标签文法) / HTML blocks(24，块级大功能) / Setext(7)。
