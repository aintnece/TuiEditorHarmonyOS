...
**（无活动批次）** — Phase 5 已全部完成。下一个需求：**图片插入支持系统图库/文件选择**（目前仅 URL 输入）。方案待定（见 Next），定方案后开工。

## Next

- **图片插入：系统图库/文件选择**（用户需求）— 当前 ImageEditor 仅 URL 文本框。需接 HarmonyOS PhotoViewPicker/DocumentViewPicker 选图，并让所选图片能在两个 WebView(WwEditor https://wweditor.local + MdPreview https://markdown.local)显示。关键设计点：图片如何存储/引用（沙箱复制+相对路径+双拦截器 vs base64 内嵌）。**方案待与用户确认**。
