...
- [x] **5.2**: WwEditor.ets WebView 容器 + `rawfile/tui-editor/editor.html` — 自定义域名 `https://wweditor.local` + onInterceptRequest 从 rawfile 加载、focusable(true) 可输入、`window.__ww` 桥 API（setMarkdown/getMarkdown/setTheme/focus）、主题适配。**编译/真机未验证**。

## In Progress

**Phase 5 批次 3: ArkTS↔JS 桥** — 在 WwEditor.ets 加 registerJavaScriptProxy（onChange 回传 markdown + selection）+ ArkTS 公开命令方法，扩展 editor.html `window.__ww` 的 exec/回调。详见 `.project/active-task.md`。
