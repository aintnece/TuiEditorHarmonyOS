...
| 文件 | 大小 | 说明 |
|------|------|------|
| `toastui-editor.min.js` | 350KB | @toast-ui/editor v3.2.2 UMD 自包含 bundle（**内含 ProseMirror**，已验证）。UMD 全局：`window.toastui.Editor` |
| `toastui-editor.min.css` | 168KB | 主样式（浅色） |
| `toastui-editor-dark.css` | 17KB | 暗色主题样式 |

> ⚠️ 注意：v3.2.2 **没有** `toastui-editor-all.min.js`（那是旧版 v2 命名）。v3 自包含入口就是 `toastui-editor.min.js`。
> rawfile 经 `getRawFileContentSync` 读取**无 8KB 限制**（8KB 只限 data: URI / loadUrl），350KB JS 没问题，与 katex 同理。

## 分批拆解（建议顺序）

- [x] 批次 1: 引擎选型 + 打包到 rawfile/tui-editor/ ✅
- [x] 批次 2: WwEditor.ets WebView 容器 + rawfile/tui-editor/editor.html ✅（规格见下，CC 已实现，Hermes 已审）
- [ ] 批次 3: ArkTS↔JS 桥（runJavaScript 命令 + registerJavaScriptProxy 回传）（**← 下一批次**）
- [ ] 批次 4: Markdown 双向转换 + 模式切换同步
- [ ] 批次 5: 集成 EditorPage（替换占位符）+ Toolbar 联动 ｜ **render 部分已先行**（EditorPage WYSIWYG 分支已渲染 WwEditor，供真机验证；剩 Toolbar 命令路由 + selection 接入）
...
**Status**: `in_progress` — 批次 1、2 完成（Hermes 已审 diff + 提交）；下一步批次 3（ArkTS↔JS 桥）
