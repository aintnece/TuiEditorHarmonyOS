...
**Status**: Phase 8.2A（Setext headings）完成。基线 exact **79.45%** (518/652)，Setext headings 20→**26/27**，Thematic breaks 15→**17**（isThematicBreak 加缩进≥4 守卫连带），error 0，hang 0，cosmetic 0，全 26 节 0 回归。改 1 文件：Blocks.ts(setextUnderlineType 返回 0/1/2 + 0-3 前导空格/连续 run/仅尾随空白 + stripTrailingSpacesTabs 去标题尾随空白 + isThematicBreak 缩进守卫)。spec: `.project/8.2A-setext-headings-spec.md`。无进行中批次。
**Resume**: 新会话读本文件 + `status.md`。**行内快赢 + HTML blocks + Setext 已做完**（Emphasis/Autolinks/Raw HTML 满分，Setext 26/27、HTML blocks 43/44）。剩余多为**块级 Blocks.ts 大功能 / 横切表 / 引用块架构**：blockquote 懒段落续行（Ex93 `> foo\nbar\n===`、Block quotes 17/25——引用块不实现 lazy continuation，是较深架构改动）/ Ex175(tight list 内 HtmlBlock 需 renderListItem 补前导 `\n`，Renderer 层)/ Ex623-624(块级 HTML 吞行内闭标签)/ Ex645(块末尾尾随空格 trim) / Lists(14/26)、List items(23/48)、Links(67/90)、Images(11/22)、Tabs(5/11) 边角；以及命名实体大表(Ex25/32/33/34 + dest/title 双因，需 vendoring ~2125 项表，碰 normalizeUri 管线)。⚠️ 块级/引用块改动碰 Blocks.ts、风险/纠缠高于行内；审 diff 必过 ArkTS 规则 + 全节回归对比。
