...
**Status**: Phase 8.2e（HTML blocks）完成。基线 exact **78.22%** (510/652)，HTML blocks 22→**43/44**，**Raw HTML 20/20 满分**，Entity 12→13 / Backslash 11→12 连带，error 0，hang 0，cosmetic 0，全 26 节 0 回归。改 2 文件：Blocks.ts(7 型分类 htmlBlockStartType/matchHtmlTagNameType/htmlBlockCloseMatch + type6 名单 + type7 复用 Inlines + isHtmlBlockInterrupt 不打断段落) + Inlines.ts(给 parseHtmlOpenTag/parseHtmlCloseTag 加 export)。spec: `.project/8.2e-html-blocks-spec.md`。无进行中批次。
**Resume**: 新会话读本文件 + `status.md`。**行内快赢 + 块级 HTML blocks 已做完**（Code spans/Entity数字/Autolinks/Hard breaks/normalizeURI/inline RawHTML/HTML blocks + Emphasis 全清，Emphasis/Autolinks/Raw HTML 满分）。剩余多为**块级 Blocks.ts 大功能 / 横切表**：Setext headings(20/27，块级，与 thematic-break/段落优先级纠缠) / Ex175(tight list 内 HtmlBlock 需 renderListItem 补前导 `\n`，Renderer 层) / Ex623-624(块级 HTML 吞行内闭标签) / Ex645(块末尾尾随空格 trim) / List items(23/48)、Lists(14/26)、Links(67/90) 边角；以及命名实体大表(Ex25/32/33/34 + dest/title 双因，需 vendoring ~2125 项表)。⚠️ 块级改动碰 Blocks.ts、风险/纠缠高于行内；审 diff 必过 ArkTS 规则 + 全节回归对比。
