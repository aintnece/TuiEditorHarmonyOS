...
**Status**: Phase 8.2h（空列表项 empty list items，簇 A）完成。基线 exact **88.80%** (579/652)，**List items 32→38**，error 0，hang 0，cosmetic 0，全 26 节 0 回归（逐节净增量 +6=+6 自洽）。改 1 文件 Blocks.ts（marker 空分支 + parseList「至多一个起始空行」+ 新增 isNonEmptyListMarker → parseParagraph/isLazyContinuable）。⚠️ 第三次「marker 识别变化→段落边界检测必须同步」联动，CC 补 isNonEmptyListMarker 防 Ex367 回归（空 marker 不打断段落），Hermes 全节回归确认安全。spec: `.project/8.2h-empty-list-items-spec.md`。上一批 8.2g 已 DevEco 编译通过。无进行中批次。
**Resume**: 新会话读本文件 + `status.md`。**已做完**：行内快赢 + HTML blocks(满分) + Setext + Blockquote 懒续行 + 图片 alt + 命名实体(满分) + tight list 块级换行 + Fenced code blocks(满分) + ATX headings(满分) + **空列表项(簇 A)**（满分节：Emphasis/Autolinks/Raw HTML/Entity/HTML blocks/Fenced code blocks/ATX headings）。剩余多为**列表前导空格 marker + 内容列模型 / 深度 Tabs / 引用式边角**：Links(69/90)、List items(38/48)、Link ref defs(16/27)、Lists(17/26)、Images(16/22)、Tabs(5/11)、Indented code(9/12)。**下一批 8.2i 候选：列表簇 B**——带前导空格(≤3)的 marker 识别（parseBulletMarker/parseOrderedMarker 现从 line[0] 读，不容忍前导空格）+ marker 后内容列 W 计算 + 后续块按列归属（Ex257/258/271/286/287/288/290/291/295/297 + Tabs Ex4/9，最大杠杆 ~+10，但血缘最广风险最高）。⚠️ **关键模式（8.2f/g/h 三次踩到）：marker/块起始识别逻辑任何变化（变严/变宽/加新形态）必须同步审 parseParagraph 推进循环 + isLazyContinuable 的对应 break 条件**（否则被拒行死循环或回归）。审 diff 必过 ArkTS 规则 + 全节回归（务必亲跑回归脚本 + 逐节净增量自洽校验 + 确认 hang=0）。
