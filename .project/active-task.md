...
- [ ] 批次 5: 集成 EditorPage（占位符已替换 render）+ **Toolbar 命令路由到 window.__ww.exec**（**← 当前批次**）
  - [x] 5.1 直接命令（粗体/斜体/删除线/行内代码/标题/引用/三列表/代码块/分割线 + 撤销/重做）— Editor.exec 路由 + WwEditor 注册 wwExec。**待真机验证**
  - [ ] 5.2 Link/Image/Table 对话框在 WYSIWYG 下走 ww.exec（addLink/addImage/addTable + payload）
...
**Status**: `in_progress` — 批次 1-4 完成并真机验证 ✓（引擎/容器/输入/双向桥/模式·文件切换同步）；下一步批次 5（Toolbar 命令在 WYSIWYG 下路由到 window.__ww.exec）
