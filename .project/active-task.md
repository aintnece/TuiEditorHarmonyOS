...
**Status**: Phase 8.2G2（实体解码接入 dest/title/info）完成。基线 exact **82.67%** (539/652)，**Entity 14→17 满分**、Links 67→69、Backslash escapes 12→13，error 0，hang 0，cosmetic 0，全 26 节 0 回归。改 2 文件：Inlines.ts(新增 export unescapeString 单趟反斜杠+实体；6 处 dest/title 解析器换 unescapeString；tryParseLinkOrImage 去冗余二次 resolve) + Blocks.ts(import unescapeString + parseCodeBlock info 解码)。命名实体 G-1+G-2 全做完。spec: `.project/8.2G2-entity-dest-title-info-spec.md`。无进行中批次。
**Resume**: 新会话读本文件 + `status.md`。**已做完**：行内快赢 + HTML blocks + Setext + Blockquote 懒续行 + 图片 alt 纯文本 + 命名实体表(文本+dest/title/info)（Emphasis/Autolinks/Raw HTML/**Entity** 满分；HTML blocks 43/44、Setext 26/27、Block quotes 24/25、Images 16/22、Links 69/90）。剩余多为**缩进/列表纠缠 / Renderer 收尾 / 引用式边角 / 深度 Tabs**：Ex238(`> foo\n    - bar` 缩进懒续行——需 parseParagraph 列表打断改缩进感知，血缘 Lists/List items)/ Ex175(tight list 内 HtmlBlock 需 renderListItem 补前导 `\n`，Renderer，极小)/ Ex573(shortcut-ref 图片未识别)/ Ex623-624(块级 HTML 吞行内闭标签)/ Ex645(块末尾尾随空格 trim) / Tabs(5/11，深度列记账 partiallyConsumedTab，不 contained、风险高)/ Lists(14/26)、List items(25/48)、Link ref defs(16/27)、Images(16/22)、Fenced(14/29) 边角。⚠️ 列表/缩进/Tabs 血缘最广；审 diff 必过 ArkTS 规则 + 全节回归（务必亲跑回归脚本，别信 CC 自报）。
