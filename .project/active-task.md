...
**Status**: Phase 8.2Img（图片 alt 纯文本化）完成。基线 exact **81.60%** (532/652)，Images 11→**16/22**，error 0，hang 0，cosmetic 0，全 26 节 0 回归（增量全在 Images）。改 1 文件：Inlines.ts(新增 plainTextOf/extractAltText + 两处图片创建把 attrs.alt 改 extractAltText；Renderer 未动)。spec: `.project/8.2Img-image-alt-plaintext-spec.md`。无进行中批次。
**Resume**: 新会话读本文件 + `status.md`。**行内快赢 + HTML blocks + Setext + Blockquote 懒续行 + 图片 alt 纯文本 已做完**（Emphasis/Autolinks/Raw HTML 满分；HTML blocks 43/44、Setext 26/27、Block quotes 24/25、Images 16/22）。剩余多为**缩进/列表纠缠 / 横切表 / Renderer 收尾 / 引用式解析边角**：Tabs(5/11，深度列记账 partiallyConsumedTab + 跨容器部分 tab 消费，绑定列表/引用块列模型，**不 contained、风险高**)/ Ex238(`> foo\n    - bar` 缩进懒续行——需 parseParagraph 列表打断改缩进感知，血缘 Lists/List items)/ 命名实体大表(Ex25/32/33/34 + dest/title 双因，vendoring ~2125 项表，碰 normalizeUri)/ Ex175(tight list 内 HtmlBlock 需 renderListItem 补前导 `\n`，Renderer 层)/ Ex573(shortcut-ref 图片未识别，引用式解析)/ Ex623-624(块级 HTML 吞行内闭标签)/ Ex645(块末尾尾随空格 trim) / Lists(14/26)、List items(25/48)、Links(67/90，多为多行/边角引用)、Link ref defs(16/27) 边角。⚠️ 列表/缩进/Tabs 改动血缘最广；审 diff 必过 ArkTS 规则 + 全节回归对比（CC 屡次把连带节增量误报「不变」，务必亲跑回归脚本）。
