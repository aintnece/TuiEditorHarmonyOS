...
**Status**: Phase 8.2i（前导空格列表 marker + 内容列 W，簇 B）完成。基线 exact **91.41%** (596/652，**突破 90%**)，**List items 38→48 满分**、Lists 17→22、连带 Indented code +1/Tabs +1，error 0，hang 0，cosmetic 0，全 26 节 0 回归（逐节净增量 +10/+5/+1/+1=17 自洽，**本会话最大单批**）。改 1 文件 Blocks.ts 仅 3 处（parseBulletMarker/parseOrderedMarker 加前导空格 + contentCol 绝对列；isNonEmptyListMarker 加 ordered start=1），**parseList 主体 0 改动**（contentCol 绝对列 + 现有 helper 绝对列语义 → 归属自动正确）。spec: `.project/8.2i-leading-space-marker-spec.md`。上一批 8.2h 已 DevEco 编译通过。无进行中批次。
**Resume**: 新会话读本文件 + `status.md`。**已做完**：行内快赢 + HTML blocks(满分) + Setext + Blockquote 懒续行 + 图片 alt + 命名实体(满分) + tight list 块级换行 + Fenced(满分) + ATX(满分) + 空列表项(簇A) + **前导空格 marker(簇B)**（满分节：Emphasis/Autolinks/Raw HTML/Entity/HTML blocks/Fenced/ATX/**List items**）。剩余 56 失败：Links(69/90,21)、Link ref defs(16/27,11)、Images(16/22,6)、Tabs(6/11,5)、Lists(22/26,4=tight/loose Ex307/315/318/319)、Indented code(10/12,2)、Code spans(2)、零碎。下一批候选：(a) **tight/loose 传播**（Lists Ex307/315/318/319，列表收尾，需碰 parseList tight/loose 判定有风险）；(b) **Links/Link-ref-def**（32 失败但多边角，性价比一般）；(c) **Tab**（Ex4/5/6/7/9/11 剩 5，partiallyConsumedTab 深度列记账，复杂）。⚠️ **关键模式（8.2f/g/h/i 四次）：marker/块起始识别变化必须同步审 parseParagraph + isLazyContinuable break 条件**。⚠️ **contentCol 绝对列设计**：列表缩进相关改动优先「统一进绝对列」，不碰 parseList 主体。审 diff 必过 ArkTS + 全节回归（亲跑回归脚本 + 逐节净增量自洽 + hang=0）。
