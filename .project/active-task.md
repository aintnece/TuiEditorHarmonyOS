...
**Status**: Phase 8.2G1（命名实体表+文本解码）完成。基线 exact **81.75%** (533/652)，Entity 13→**14**，新增 `commonmark/HtmlEntities.ts`（Hermes vendoring 2125 项 HTML5 实体表，ASCII 码点打包），error 0，hang 0，cosmetic 0，全 26 节 0 回归。改 1 文件 + 新增 1 数据文件：Inlines.ts(惰性 Map 构建 getNamedEntityMap/lookupNamedEntity + tryParseEntity 命名分支换全表查找；修 nbsp→真 NBSP) + HtmlEntities.ts(数据)。spec: `.project/8.2G1-named-entities-text-spec.md`。无进行中批次。
**Resume**: 新会话读本文件 + `status.md`。**行内快赢 + HTML blocks + Setext + Blockquote 懒续行 + 图片 alt 纯文本 + 命名实体表(文本) 已做完**（Emphasis/Autolinks/Raw HTML 满分；HTML blocks 43/44、Setext 26/27、Block quotes 24/25、Images 16/22、Entity 14/17）。**下一步首选 G-2**：实体表已就位，把实体解码接到 link dest/title + code fence info string（Ex32 `[foo](/f&ouml;&ouml; "f&ouml;&ouml;")`、Ex33 ref-def、Ex34 ` ``` f&ouml;&ouml;`）——需在 dest/title/info 提取后跑 unescape(backslash+entity)，dest 解码后再 normalizeUri（注意顺序：decode → normalizeUri）；复用 lookupNamedEntity，可连带 Links/Images/Link-ref-def/Fenced 含实体的双因例。⚠️ 碰 normalizeUri 管线属横切，回归对比加倍小心。其它剩余：Ex238 缩进懒续行(碰 parseParagraph 列表打断)/ Ex175 Renderer / Ex623-624 / Ex645 / Tabs(深度列记账,不建议) / Lists·List items·Links 边角。审 diff 必过 ArkTS 规则 + 全节回归（CC 屡次把连带节增量误报「不变」，务必亲跑回归脚本）。
