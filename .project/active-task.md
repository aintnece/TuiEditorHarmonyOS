...
**Status**: Phase 8.2e-6（inline Raw HTML）完成。基线 exact **74.39%** (485/652)，Raw HTML 6→18，**Emphasis 132/132 满分**，Hard breaks 12→14，HTML blocks/Links/Code spans 连带，error 0，hang 0，全 26 节 0 回归。改 3 文件：Node.ts(加 HtmlInline 枚举) + Renderer.ts(加逐字 case) + Inlines.ts(tryParseHtmlTag + 6 产生式 char-loop)。无进行中批次。
**Resume**: 新会话读本文件 + `status.md`。**行内快赢已基本做完**（Code spans/Entity数字/Autolinks/Hard breaks/normalizeURI/inline RawHTML + Emphasis 全清）。剩余多为**块级 Blocks.ts 大功能**：HTML blocks(块级 7 型起始条件，最大但最重) / Setext(7，块级，与 thematic-break/缩进/blockquote/段落优先级纠缠) / Ex623-624(块级 HTML 吞了行内闭标签) / Ex645(块末尾尾随空格 trim)；以及命名实体大表(Ex25/32/33/34，Hermes vendoring ~2125 项表 + dest/title/info-string 上下文解码，dest 解码会碰 normalizeUri 管线)。⚠️ 块级改动碰 Blocks.ts、风险/纠缠高于行内；审 diff 必过 ArkTS 规则 + 全节回归对比。
