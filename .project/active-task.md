...
**Status**: Phase 8.2g（ATX headings `#` 标题）完成。基线 exact **87.88%** (573/652)，**ATX headings 12→18 满分**，error 0，hang 0，cosmetic 0，全 26 节 0 回归（逐节净增量 +6=+6 自洽）。改 1 文件 Blocks.ts（具名 class AtxOpen + 2 helper 重写 parseHeading + 入口第45行）。⚠️ CC 超 spec 范围改了 parseParagraph 第568行（入口变严后被拒的非标题 `#` 行落段落会死循环，必要联动修复），Hermes 全节回归确认安全。spec: `.project/8.2g-atx-headings-spec.md`。上一批 8.2f 已 DevEco 编译通过。无进行中批次。
**Resume**: 新会话读本文件 + `status.md`。**已做完**：行内快赢 + HTML blocks(满分) + Setext + Blockquote 懒续行 + 图片 alt + 命名实体(满分) + tight list 块级换行 + Fenced code blocks(满分) + **ATX headings(满分)**（满分节：Emphasis/Autolinks/Raw HTML/Entity/HTML blocks/Fenced code blocks/ATX headings）。剩余多为**列表内容缩进模型 / 深度 Tabs / 引用式边角**：Links(69/90)、List items(32/48)、Link ref defs(16/27)、Lists(17/26)、Images(16/22)、Tabs(5/11)、Indented code(9/12)。下一批候选：Indented code(Ex111/117 空白行归一化，小独立批 ~+2) 或 Links/Link-ref-def(边角多、性价比一般) 或啃列表缩进模型(List items+Lists+Tabs 血缘最广、风险最高)。⚠️ **「入口检测变严」类改动（startsWith→语义校验）必须同步查 parseParagraph 推进循环 break 条件**（否则被拒行死循环，8.2f/8.2g 两次踩到）。列表缩进模型+Tabs 血缘最广；审 diff 必过 ArkTS 规则 + 全节回归（务必亲跑回归脚本 + 逐节净增量自洽校验）。
